...
module savestate_ui (
    ...
    input             allow_load,    // 1 when restoring is permitted (0 in RA hardcore)
...
always @(posedge clk) begin
    ...
    if (~old_osd_load & OSD_saveload[1] & allow_ss & allow_load && (cooldown_cnt == 0)) begin
        ss_load      <= 1;
        ss_info_req  <= 1;
    ...
    if (kbd_load & allow_ss & allow_load && (cooldown_cnt == 0)) begin
        ss_load      <= 1;
        ss_info_req  <= 1;
    ...
    if (joySS) begin
        ...
        if (~joyUp_r & joyUp & ~joyLeft & ~joyRight & allow_ss & allow_load && (cooldown_cnt == 0)) begin
            ss_load       <= 1;
            ss_info_req   <= 1;
            ss_info       <= 8'd7 + {slot, 1'b0};
            ss_combo_done <= 1;
